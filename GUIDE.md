...
Домен запоминается. Контейнеры создадутся автоматически при добавлении пользователей.
...
```bash
# Создать пользователя (контейнеры для ya.ru, google.com, cloudflare.com создадутся автоматически)
mtpx user add alice
