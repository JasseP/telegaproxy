...
Рекомендуется создать пользователя **до** добавления доменов. При добавлении домена автоматически создаются контейнеры для всех существующих пользователей.
...
Домены — это серверы, под которые прокси будет маскироваться. При добавлении домена создаются контейнеры для всех пользователей.
...
**Что происходит:**
- Для **каждого пользователя** создаётся контейнер `mtproto-<домен>-<username>` с уникальным секретом
- Если пользователей ещё нет — домен добавляется в список, но контейнеры не создаются (создадутся при добавлении пользователя)
