...
# ─────────────────────────────────────────────────────────────────────────────
# Шаг 3.5: Настройка git (если репозиторий)
# ─────────────────────────────────────────────────────────────────────────────
# install.sh меняет права файлов (chmod). Git по умолчанию отслеживает
# изменения executable bit, что приводит к ложным конфликтам при git pull.
# Устанавливаем core.fileMode false — git будет реагировать только на
# изменения содержимого, а не прав.
# Это одноразовая настройка, которая решает проблему «локальные изменения»
# после запуска install.sh и позволяет бесшовно работать по схеме:
